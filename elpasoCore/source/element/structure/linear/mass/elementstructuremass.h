...
#ifndef INFAM_ELEMENTSTRUCTURE_MASS_H
#define INFAM_ELEMENTSTRUCTURE_MASS_H

#include "../elementstructurelinear.h"

//! @brief Point mass
class cElementStructureMass : public cElementStructureLinear,
                              private tCounter<cElementStructureMass> {
 protected:
 ...
 public:
  //! @brief Constructor
  cElementStructureMass();
  //! @brief Copy constructor
  cElementStructureMass(const cElementStructureMass &other);
  //! @brief Destructor
  ~cElementStructureMass();
  ...
  eElementType getElementType(void) const { return Pointmass; }

  //! @brief return the number of instances of this object
  static size_t howMany(void) {
    return tCounter<cElementStructureMass>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureMass::computeStressesCells() not reasonable.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureMass::computeStressesNodes() not reasonable.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureMass &other) {
