...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGBCRZ_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGBCRZ_H

#include "../elementstructurelinear.h"

class cElementStructureSpringBCrz
    : public cElementStructureLinear,
      private tCounter<cElementStructureSpringBCrz> {
 protected:
 ...
 public:
  cElementStructureSpringBCrz();
  cElementStructureSpringBCrz(const cElementStructureSpringBCrz &other);
  ~cElementStructureSpringBCrz();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringBCrz>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpringBCrz::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringBCrz::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void getLocalCoordinatesOfElementsNodes(cMatrix &xyz) {
    trace(
        "ERROR: "
        "cElementStructureSpringBCrz::getLocalCoordinatesOfElementsNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringBCrz &other) {
