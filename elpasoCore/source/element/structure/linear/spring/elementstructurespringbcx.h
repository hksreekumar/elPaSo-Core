...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGBCX_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGBCX_H

#include "../elementstructurelinear.h"

class cElementStructureSpringBCx
    : public cElementStructureLinear,
      private tCounter<cElementStructureSpringBCx> {
 protected:
 ...
 public:
  cElementStructureSpringBCx();
  cElementStructureSpringBCx(const cElementStructureSpringBCx &other);
  ~cElementStructureSpringBCx();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringBCx>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpringBCx::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringBCx::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void getLocalCoordinatesOfElementsNodes(cMatrix &xyz) {
    trace(
        "ERROR: "
        "cElementStructureSpringBCx::getLocalCoordinatesOfElementsNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringBCx &other) {
