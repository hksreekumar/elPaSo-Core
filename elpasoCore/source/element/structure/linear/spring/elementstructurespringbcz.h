...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGBCZ_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGBCZ_H

#include "../elementstructurelinear.h"

class cElementStructureSpringBCz
    : public cElementStructureLinear,
      private tCounter<cElementStructureSpringBCz> {
 protected:
 ...
 public:
  cElementStructureSpringBCz();
  cElementStructureSpringBCz(const cElementStructureSpringBCz &other);
  ~cElementStructureSpringBCz();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringBCz>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpringBCz::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringBCz::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void getLocalCoordinatesOfElementsNodes(cMatrix &xyz) {
    trace(
        "ERROR: "
        "cElementStructureSpringBCz::getLocalCoordinatesOfElementsNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringBCz &other) {
