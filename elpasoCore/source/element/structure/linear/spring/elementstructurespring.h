...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRING_H
#define INFAM_ELEMENTSTRUCTURE_SPRING_H

#include "../elementstructurelinear.h"

class cElementStructureSpring : public cElementStructureLinear,
                                private tCounter<cElementStructureSpring> {
 protected:
 ...
 public:
  cElementStructureSpring();
  cElementStructureSpring(const cElementStructureSpring &other);
  ~cElementStructureSpring();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpring>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpring::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpring::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpring &other) {
