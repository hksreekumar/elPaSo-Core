...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGBCRX_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGBCRX_H

#include "../elementstructurelinear.h"

class cElementStructureSpringBCrx
    : public cElementStructureLinear,
      private tCounter<cElementStructureSpringBCrx> {
 protected:
 ...
 public:
  cElementStructureSpringBCrx();
  cElementStructureSpringBCrx(const cElementStructureSpringBCrx &other);
  ~cElementStructureSpringBCrx();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringBCrx>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpringBCrx::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringBCrx::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void getLocalCoordinatesOfElementsNodes(cMatrix &xyz) {
    trace(
        "ERROR: "
        "cElementStructureSpringBCrx::getLocalCoordinatesOfElementsNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringBCrx &other) {
