...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGZ_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGZ_H

#include "../elementstructurelinear.h"

class cElementStructureSpringz : public cElementStructureLinear,
                                 private tCounter<cElementStructureSpringz> {
 protected:
 ...
 public:
  cElementStructureSpringz();
  cElementStructureSpringz(const cElementStructureSpringz &other);
  ~cElementStructureSpringz();
  ...
  eElementType getElementType(void) const { return Spring; }

  //! return the number of instances of this object
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringz>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpringz::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringz::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringz &other) {
