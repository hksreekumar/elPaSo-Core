...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGBC_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGBC_H

#include "../elementstructurelinear.h"

class cElementStructureSpringBC : public cElementStructureLinear,
                                  private tCounter<cElementStructureSpringBC> {
 protected:
 ...
 public:
  cElementStructureSpringBC();
  cElementStructureSpringBC(const cElementStructureSpringBC &other);
  ~cElementStructureSpringBC();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringBC>::howMany();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringBC::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void getLocalCoordinatesOfElementsNodes(cMatrix &xyz) {
    trace(
        "ERROR: "
        "cElementStructureSpringBC::getLocalCoordinatesOfElementsNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringBC &other) {
