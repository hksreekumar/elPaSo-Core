...
#ifndef INFAM_ELEMENTSTRUCTURE_SPRINGBCRY_H
#define INFAM_ELEMENTSTRUCTURE_SPRINGBCRY_H

#include "../elementstructurelinear.h"

class cElementStructureSpringBCry
    : public cElementStructureLinear,
      private tCounter<cElementStructureSpringBCry> {
 protected:
 ...
 public:
  cElementStructureSpringBCry();
  cElementStructureSpringBCry(const cElementStructureSpringBCry &other);
  ~cElementStructureSpringBCry();
  ...
  static size_t howMany(void) {
    return tCounter<cElementStructureSpringBCry>::howMany();
  }
  ...
  void computeStressesCells(Vec &fullSolution, Vec &stresses,
                            Vec &stressesSec2) {
    trace(
        "ERROR: cElementStructureSpringBCry::computeStressesCells() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void computeStressesNodes(Vec &fullSolution, Vec &stresses) {
    trace(
        "ERROR: cElementStructureSpringBCry::computeStressesNodes() not "
        "implemented, yet.");
    ExitApp();
  }
  ...
  void getLocalCoordinatesOfElementsNodes(cMatrix &xyz) {
    trace(
        "ERROR: "
        "cElementStructureSpringBCry::getLocalCoordinatesOfElementsNodes() not "
        "implemented, yet.");
    ExitApp();
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const cElementStructureSpringBCry &other) {
