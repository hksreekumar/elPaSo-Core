...
#include "ncelementinterface.h"

cNCElementInterface::cNCElementInterface(short NumberOfNodes)
...
cNCElementInterface::cNCElementInterface(const cNCElementInterface &other)
    : cElement(other) {
...
cNCElementInterface::~cNCElementInterface() {
  for (int k = 0; k < getNumberOfNodes(); k++) m_MatchingNodes[k] = NULL;
}

cNode *cNCElementInterface::getMatchingNode(int Index) const {
...
void cNCElementInterface::setMatchingNode(int Index, cNode *ptrNode) {
#ifdef PETSC_USE_DEBUG
...
#if 0
void cNCElementInterface::applyBoundaryConditions(cElementMatrix &C, cElementMatrix &CT)
{
...
#if 0
void cNCElementInterface::addContributionFrequencyDomain(const PetscReal &omega, Mat &K, Vec &F, int step)
{
...
addContribution(K, K, factorC, factorCT,step);

}
#endif

#if 0
void cNCElementInterface::addContributionTimeDomain(Mat &K, Mat &M)
{
  const PetscScalar rho = getRhoF();
...
std::ostream &cNCElementInterface::write(std::ostream &os) const {
